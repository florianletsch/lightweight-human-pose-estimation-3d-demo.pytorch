...
def extract_poses(heatmaps, pafs, upsample_ratio):
	...
	for pose_entry in pose_entries:
        if len(pose_entry) == 0:
            continue
        pose_keypoints = np.ones((num_keypoints * 3 + 1), dtype=np.int32) * -1
        for kpt_id in range(num_keypoints):
            if pose_entry[kpt_id] != -1.0:
                pose_keypoints[kpt_id * 3 + 0] = int(all_keypoints[int(pose_entry[kpt_id]), 0])
                pose_keypoints[kpt_id * 3 + 1] = int(all_keypoints[int(pose_entry[kpt_id]), 1])
                pose_keypoints[kpt_id * 3 + 2] = int(all_keypoints[int(pose_entry[kpt_id]), 2])
